...
class AllureReporter {
  ...
  get currentStep() {
    if (this.runningItems.length === 0) return null;
  ...
  get currentTest() {
    if (this.runningItems.length === 0) throw new Error("No active test");
  ...
  prerequest(err, args) {
    if (
      ...
      this.runningItems[
        this.runningItems.length - 1
      ].pm_item.prerequest = args.executions[0].script.exec.join("\n");
    }
  }
  ...
  test(err, args) {
    if (
      ...
      this.runningItems[
        this.runningItems.length - 1
      ].pm_item.testscript = args.executions[0].script.exec.join("\n");
  }
  ...
  console(err, args) {
    ...
    if (args.level) {
      if (
        ...
        this.runningItems[
          this.runningItems.length - 1
        ].pm_item.console_logs = [];
        this.runningItems[
          this.runningItems.length - 1
        ].pm_item.console_logs.push(
          `level: ${args.level}, messages: ${args.messages}`
        );
      } else {
        this.runningItems[
          this.runningItems.length - 1
  ...
  assertion(err, args) {
    ...
    if (err) {
      ...
      this.runningItems[
        this.runningItems.length - 1
      ].pm_item.failedAssertions.push(args.assertion);
  ...
  beforeItem(err, args) {
      ...
      if (this.reporterOptions.collectionAsParentSuite === true) {
        if (fullName.indexOf("/") > 0) {
          ...
          if (numFolders > 0) {
            ...
            if (numFolders > 1) subSuites = fullName.split("/").slice(1);
          }
        } else {
          suite = fullName;
        }
    ...
    if (path !== undefined) allure_test.addLabel(LabelName.STORY, path);
  ...
  failTestCase(allure_test, error) {
    ...
    this.endTest(allure_test, status, {
  ...
  item(err, args) {
    ...
    const requestDataURL =
      rItem.pm_item.request_data.method +
      " - " +
      rItem.pm_item.request_data.url;
    let bodyModeProp = "";
    let bodyModePropObj;
    ...
    const reqTableStr = html` <table>
      <tr>
        <th
          style="border: 1px solid #dddddd;text-align: left;padding: 8px;color:Orange;"
        >
          ${bodyModeProp}
        </th>
        <td style="border: 1px solid #dddddd;text-align: left;padding: 8px;">
          <pre style="color:Orange"> <b> ${bodyModePropObj} </b> </pre>
        </td>
      </tr>
    </table>`;

    const responseCodeStatus =
      rItem.pm_item.response_data.code +
      " - " +
      rItem.pm_item.response_data.status;
  ...
  escape(string) {
    return string
      ...
      .replace(
        /[\u0100-\uffff]/g,
        (c) => `|0x${c.charCodeAt(0).toString(16).padStart(4, "0")}`
      );
  }
}
